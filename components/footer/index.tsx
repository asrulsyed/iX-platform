...
const Footer = () => {
  ...
  return (
    <footer className="sticky px-6 py-6 text-sm border-t z-50 pt-7 bg-bgColor border-borderColor text-fontColor h-footerHeight">
      <div className="flex items-center justify-center">
        <div className="max-w-[1080px] w-full">
          <nav className="h-[34px] flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative flex items-center justify-around w-24 h-8 gap-2 border rounded-full border-borderColor">
                <button
                  className={`${theme === 0 ? 'border-borderColor' : 'border-transparent'} w-8 h-8 rounded-full border flex items-center justify-center absolute left-0`}
                  onClick={() => setTheme(0)}
                  >
                  <RiSaveFill className="w-4 h-4" />
                </button>
                <button 
                className={`${theme === 1 ? 'border-borderColor' : 'border-transparent'} w-8 h-8 rounded-full border flex items-center justify-center absolute`}
                onClick={() => setTheme(1)}
                >
                  <RiSunLine className="w-4 h-4" />
                </button>
                <button 
                className={`${theme === 2 ? 'border-borderColor' : 'border-transparent'} w-8 h-8 rounded-full border flex items-center justify-center absolute right-0`}
                onClick={() => setTheme(2)}>
          ...
          <ul className="grid items-center justify-between grid-cols-3 gap-4 mt-6 min-[850px]:flex pb-10">
            ...
            <li className="transition-colors duration-200 ease-in-out cursor-pointer hover:text-fontHover hidden xl:block">
              <Link href="">Home</Link>
            </li>
            <li className="transition-colors duration-200 ease-in-out cursor-pointer hover:text-fontHover hidden xl:block">
            ...
            <li className="relative" ref={legalRef}>
              ...
              <div
                className={`absolute -top-4 -translate-y-full border border-borderColor rounded-md w-[230px] right-0 bg-bgColor p-2 flex flex-col items-center transition-opacity duration-300 ease-in-out
                  ${legal === true ? 'visible opacity-1' : 'invisible opacity-0'}`}
              >
